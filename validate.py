    ...
    for file_path in file_paths:
        try:
            ...
            with open(file_path) as json_file:
                loaded_data.append(json.load(json_file))
...
def main():
    ...
    all_valid = True  # Track overall validation result
    ...
    for file_path in changed_files:
        ...
        try:
            ...
            print(f"{file_path}: JSON is valid against the schema.")
        except jsonschema.exceptions.ValidationError as e:
            print(f"{file_path}: JSON is not valid against the schema.")
            print(e)
            all_valid = False
    
    if not all_valid:
        sys.exit(1)  # Exit with failure status code if any validation fails
